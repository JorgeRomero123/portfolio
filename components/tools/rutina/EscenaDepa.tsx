 ...
 */
function Item({
  activo, nuevo, children, origen,
}: {
  activo: boolean; nuevo?: boolean; children: React.ReactNode; origen: string;
}) {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-gray-200 bg-[#f7f1e8]">
      <svg viewBox="0 0 400 250" className="block w-full" role="img" aria-label={`Tu depa en el nivel ${nivel}`}>
        ...
        {/* ventana — nivel 6 */}
        <Item activo={has('ventana')} nuevo={nuevo === 'ventana'} origen="center">
          <g>
        ...
        <Item activo={has('poster')} nuevo={nuevo === 'poster'} origen="center">
          <g>
        ...
        {/* repisa — nivel 5 */}
        <Item activo={has('repisa')} nuevo={nuevo === 'repisa'} origen="center">
          <g>
        ...
        {/* alfombra — nivel 4 */}
        <Item activo={has('alfombra')} nuevo={nuevo === 'alfombra'} origen="center">
        ...
        <Item activo={has('tapete')} nuevo={nuevo === 'tapete'} origen="center">
          <g>
        ...
        {/* lámpara — nivel 3 */}
        <Item activo={has('lampara')} nuevo={nuevo === 'lampara'} origen="bottom">
          <g>
        ...
        {/* planta — nivel 2 */}
        <Item activo={has('planta')} nuevo={nuevo === 'planta'} origen="bottom">
          <g>
        ...
        {/* guitarra — nivel 7 */}
        <Item activo={has('guitarra')} nuevo={nuevo === 'guitarra'} origen="bottom">
        ...
        {/* tocadiscos — nivel 8 */}
        <Item activo={has('tocadiscos')} nuevo={nuevo === 'tocadiscos'} origen="bottom">
          <g>
        ...
        {/* pesa rusa — nivel 11 */}
        <Item activo={has('pesa')} nuevo={nuevo === 'pesa'} origen="bottom">
          <g>
        ...
        {/* gato — nivel 10 */}
        <Item activo={has('gato')} nuevo={nuevo === 'gato'} origen="bottom">
          <g>
      ...
      {/* próximo desbloqueo */}
      <div className="flex items-center justify-between gap-3 border-t border-gray-200 bg-white/80 px-4 py-2.5 backdrop-blur">
        <span className="text-xs text-gray-600">
          Tu depa · nivel {nivel} · {tiene.size} de {ITEMS_DEPA.length} cosas
        </span>
        {proximo ? (
          <span className="text-xs font-medium text-gray-500">
            🔒 nivel {proximo.nivel}: {proximo.nombre}
          </span>
        ) : (
          <span className="text-xs font-medium text-emerald-700">Depa completo 🎉</span>
        )}
      </div>
    </div>
  );
