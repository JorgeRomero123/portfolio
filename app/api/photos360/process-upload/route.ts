...
/**
 * Process Uploaded 360° Photo
 *
 * This endpoint handles post-upload processing of 360° images:
 ...
 * 6. Updates photos360.json
 *
 * This runs AFTER the client has uploaded the file to R2 via pre-signed URL.
 */
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`Processing uploaded 360° image: ${key}`);
    ...
    // Original key: photos360/uploads/{uuid}.{ext}
    // New keys: photos360/{uuid}.webp, photos360/{uuid}-thumb.webp
    const keyParts = key.split('/');
    const filename = keyParts[keyParts.length - 1];
    const uniqueId = filename.split('.')[0];

    const imageKey = `photos360/${uniqueId}.webp`;
    const thumbnailKey = `photos360/${uniqueId}-thumb.webp`;
    ...
    // Step 5: Update photos360.json
    const contentPath = path.join(process.cwd(), 'content', 'photos360.json');
    ...
    console.log(`Updated photos360.json with new photo: ${newPhoto.id}`);
    ...
    return NextResponse.json(
      {
        ...
        message: '360° photo processed and added successfully',
      },
      { status: 201 }
    );
  } catch (error) {
